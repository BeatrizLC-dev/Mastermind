...
function verifyColor(playerColors, computerColors) {
    ...
    // Compter les occurrences de chaque couleur dans computerColors
    for (let color of computerColors) {
        if (colorCount[color]) {
            colorCount[color]++;
        } else {
            colorCount[color] = 1;
    ...
    for (let i = 0; i < computerColors.length; i++) {
        ...
        if (computerColors[i] === playerColors[i]) {
            ...
            colorCount[computerColors[i]]--; // Décrémenter le compteur de la couleur
        }
    }
    ...
    for (let i = 0; i < playerColors.length; i++) {
        ...
        if (computerColors[i] !== playerColors[i] && colorCount[playerColors[i]] > 0) {
            wrongPositionColors++;
            colorSelect.style.backgroundColor = '#FFD580'; // Bonne couleur mais mauvaise position
            colorCount[playerColors[i]]--; // Décrémenter le compteur de la couleur
